 ...
 * Calls the Python prediction script and returns structured JSON
 */
 ...
 * @param {Object} params - { symbol: string, hours: number }
...
async function predictPrice(params) {
  ...
  return new Promise((resolve, reject) => {
    ...
    proc.on('close', (code) => {
      if (code === 0) {
        // Parse output: "Predicted price of BTC at 2026-07-20 18:00 HKT: $64,063.70"
        const match = stdout.match(/Predicted price of (\w+) at ([\d\-: ]+) HKT: \$([\d,]+\.\d+)/);
        if (match) {
          const [, coin, timestamp, priceStr] = match;
